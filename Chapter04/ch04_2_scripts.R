...
# 3) 임의의 분포에서 추출된 표본평균 x_bar 의 분포
set.seed(9)
t <- 10
p <- 0.1
x <- 0:10
...
par(mfrow=c(2,2))
hist(b.2.mean, prob=TRUE, xlim=c(0,4), main="표본 크기: 2", col="orange", border="red")
x1 <- seq(min(b.2.mean), max(b.2.mean), length=1000)
y1 <- dnorm(x=x1, mean=1, sd=(sqrt(0.9)/sqrt(2)))
lines(x1, y1, lty=2, col="blue")

hist(b.4.mean, prob=TRUE, xlim=c(0,4), main="표본 크기: 4", col="orange", border="red")
x2 <- seq(min(b.4.mean), max(b.4.mean), length=1000)
y2 <- dnorm(x=x2, mean=1, sd=(sqrt(0.9)/sqrt(4)))
lines(x2, y2, lty=2, col="blue")

hist(b.32.mean, prob=TRUE, xlim=c(0.5,1.5), main="표본 크기: 32", col="orange", border="red")
x3 <- seq(min(b.32.mean), max(b.32.mean), length=1000)
y3 <- dnorm(x=x3, mean=1, sd=(sqrt(0.9)/sqrt(32)))
lines(x3, y3, lty=2, col="blue")

hist(b.64.mean, prob=TRUE, xlim=c(0.5,1.5), main="표본 크기: 64", col="orange", border="red")
